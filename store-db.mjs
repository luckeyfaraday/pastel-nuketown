...
export function openStoreDatabase(path, options = {}) {
  ...
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS currency_topups (
    ...
    CREATE INDEX IF NOT EXISTS currency_topups_by_user
      ON currency_topups(user_id, credited_at);
    ...
    CREATE TABLE IF NOT EXISTS currency_ledger (
      ...
      kind TEXT NOT NULL CHECK (
        kind IN ('topup', 'purchase', 'payment_reversal', 'payment_restore')
      ),
      reference_id TEXT NOT NULL UNIQUE,
      created_at INTEGER NOT NULL
    ) STRICT;

    CREATE INDEX IF NOT EXISTS currency_ledger_by_user
      ON currency_ledger(user_id, created_at, id);
  ...
  const statements = {
    ...
    `),
    currencyBalance: database.prepare(`
      SELECT balance FROM currency_wallets WHERE user_id = ?
    `),
    createCurrencyWallet: database.prepare(`
      INSERT OR IGNORE INTO currency_wallets (user_id, balance, updated_at)
      VALUES (?, 0, ?)
    `),
    changeCurrencyBalance: database.prepare(`
      UPDATE currency_wallets
      SET balance = balance + ?, updated_at = ?
      WHERE user_id = ?
      RETURNING balance
    `),
    spendCurrency: database.prepare(`
      UPDATE currency_wallets
      SET balance = balance - ?, updated_at = ?
      WHERE user_id = ? AND balance >= ?
      RETURNING balance
    `),
    insertCurrencyLedger: database.prepare(`
      INSERT INTO currency_ledger (
        user_id, amount, kind, reference_id, created_at
      ) VALUES (?, ?, ?, ?, ?)
    `),
    insertCurrencyTopup: database.prepare(`
      INSERT OR IGNORE INTO currency_topups (
        checkout_session_id, payment_intent_id, user_id, pack_id, amount,
        credited_at, reversed_at, revocation_id
      ) VALUES (?, ?, ?, ?, ?, ?, NULL, NULL)
    `),
    latestCurrencyTopup: database.prepare(`
      SELECT COUNT(*) AS topup_count
      FROM currency_topups
      WHERE user_id = ? AND pack_id = ?
    `),
    reverseCurrencyTopupByPayment: database.prepare(`
      UPDATE currency_topups
      SET reversed_at = ?, revocation_id = ?
      WHERE payment_intent_id = ? AND reversed_at IS NULL
      RETURNING checkout_session_id, user_id, amount, revocation_id
    `),
    reverseCurrencyTopupByCheckout: database.prepare(`
      UPDATE currency_topups
      SET reversed_at = ?, revocation_id = ?
      WHERE checkout_session_id = ? AND reversed_at IS NULL
      RETURNING checkout_session_id, user_id, amount, revocation_id
    `),
    restoreCurrencyTopupByPayment: database.prepare(`
      UPDATE currency_topups
      SET reversed_at = NULL, revocation_id = NULL
      WHERE payment_intent_id = ? AND reversed_at IS NOT NULL
      RETURNING checkout_session_id, user_id, amount, revocation_id
    `),
    restoreCurrencyTopupByCheckout: database.prepare(`
      UPDATE currency_topups
      SET reversed_at = NULL, revocation_id = NULL
      WHERE checkout_session_id = ? AND reversed_at IS NOT NULL
      RETURNING checkout_session_id, user_id, amount, revocation_id
    `),
    currencyPurchase: database.prepare(`
      INSERT OR IGNORE INTO currency_purchases (
        user_id, cosmetic_id, amount, purchased_at
      ) VALUES (?, ?, ?, ?)
    `),
  ...
  function currencyBalance(userId) {
    const row = statements.currencyBalance.get(userId);
    return row ? Number(row.balance) : 0;
  }

  function creditCurrencyTopup({
    userId,
    packId,
    amount,
    checkoutSessionId,
    ...
    if (!Number.isSafeInteger(amount) || amount <= 0 || !checkoutSessionId)
      throw new Error('A currency top-up needs a positive amount and checkout session.');
    return withImmediateTransaction(() => {
      const inserted = statements.insertCurrencyTopup.run(
        checkoutSessionId,
        paymentIntentId,
        userId,
        packId,
      ...
      if (!inserted) return { credited: false, balance: currencyBalance(userId) };
      statements.createCurrencyWallet.run(userId, creditedAt);
      const row = statements.changeCurrencyBalance.get(amount, creditedAt, userId);
      statements.insertCurrencyLedger.run(
        userId,
        amount,
        'topup',
        `topup:${checkoutSessionId}`,
        creditedAt
      );
      return { credited: true, balance: Number(row.balance) };
    });
  }

  function currencyTopupVersion(userId, packId) {
    const row = statements.latestCurrencyTopup.get(userId, packId);
    return row ? Number(row.topup_count) : 0;
  }

  function reverseCurrencyTopup({
    paymentIntentId = null,
    checkoutSessionId = null,
    reversedAt = Date.now()
  }) {
    const statement = paymentIntentId
      ? statements.reverseCurrencyTopupByPayment
      : (checkoutSessionId ? statements.reverseCurrencyTopupByCheckout : null);
    const value = paymentIntentId || checkoutSessionId;
    if (!statement) return { reversed: false };
    return withImmediateTransaction(() => {
      const revocationId = String(makeRevocationId());
      const row = statement.get(reversedAt, revocationId, value);
      if (!row) return { reversed: false };
      statements.createCurrencyWallet.run(row.user_id, reversedAt);
      const wallet = statements.changeCurrencyBalance.get(
        -Number(row.amount), reversedAt, row.user_id
      );
      statements.insertCurrencyLedger.run(
        row.user_id,
        -Number(row.amount),
        'payment_reversal',
        `reversal:${row.checkout_session_id}:${row.revocation_id}`,
        reversedAt
      );
      return {
        reversed: true,
  ...
  function restoreCurrencyTopup({
    paymentIntentId = null,
    checkoutSessionId = null,
    restoredAt = Date.now()
  }) {
    const statement = paymentIntentId
      ? statements.restoreCurrencyTopupByPayment
      : (checkoutSessionId ? statements.restoreCurrencyTopupByCheckout : null);
    const value = paymentIntentId || checkoutSessionId;
    if (!statement) return { restored: false };
    return withImmediateTransaction(() => {
      const row = statement.get(value);
      if (!row) return { restored: false };
      statements.createCurrencyWallet.run(row.user_id, restoredAt);
      const wallet = statements.changeCurrencyBalance.get(
        Number(row.amount), restoredAt, row.user_id
      );
      statements.insertCurrencyLedger.run(
        row.user_id,
        Number(row.amount),
        'payment_restore',
        `restore:${row.checkout_session_id}:${restoredAt}`,
        restoredAt
      );
      return {
        restored: true,
  ...
  /* The balance debit, purchase receipt, entitlement, and any battle-pass
     unlocks commit together. Two tabs therefore cannot spend the same coins,
     and retrying a successful request cannot charge twice. */
  function purchaseWithCurrency({
    userId,
    cosmeticId,
    amount,
    ...
    if (!Number.isSafeInteger(amount) || amount <= 0)
      throw new Error('A currency purchase needs a positive integer amount.');
    return withImmediateTransaction(() => {
      if (hasEntitlement(userId, cosmeticId))
        return { purchased: false, reason: 'already_owned', balance: currencyBalance(userId) };
      statements.createCurrencyWallet.run(userId, purchasedAt);
      const wallet = statements.spendCurrency.get(amount, purchasedAt, userId, amount);
      if (!wallet)
        return { purchased: false, reason: 'insufficient_funds', balance: currencyBalance(userId) };
      const receipt = statements.currencyPurchase.run(
        userId, cosmeticId, amount, purchasedAt
      ).changes > 0;
      if (!receipt)
        throw new Error('A currency purchase receipt exists without an active entitlement.');
      grantEntitlement({ userId, cosmeticId, grantedAt: purchasedAt });
      statements.insertCurrencyLedger.run(
        userId,
        -amount,
        'purchase',
        `purchase:${userId}:${cosmeticId}`,
        purchasedAt
      );
      const unlocked = apply({ userId, cosmeticId, purchasedAt });
      return {
        purchased: true,
        balance: Number(wallet.balance),
        rewardsUnlocked: Array.isArray(unlocked) ? unlocked : []
      };
    });
  }
  ...
  return {
    ...
    currencyBalance,
    currencyTopupVersion,
    creditCurrencyTopup,
    reverseCurrencyTopup,
    restoreCurrencyTopup,
