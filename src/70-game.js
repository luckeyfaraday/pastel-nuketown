...
function initInput() {
  ...
  document.addEventListener('pointerlockchange', () => {
    /* A phone never holds the lock, so on touch this would fire once on
       the way in and pause the match permanently with no way back. */
    if (TOUCH.on) return;
...
function requestLock() {
  try {
    if (TOUCH.on) return;                        // touch play never locks
