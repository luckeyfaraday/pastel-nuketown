...

{
  ...
  for f in mapspec.js map-registry.js bots.js net-protocol.js; do
    echo "<script>/* ===== $f ===== */"
    cat "$f"
    echo '</script>'
  done

  for f in src/10-core.js src/20-world.js src/30-physics.js src/35-map-runtime.js src/40-weapons.js \
